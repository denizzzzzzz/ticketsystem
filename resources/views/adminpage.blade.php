@extends('layouts.base')

@section('content')


    <div class="bg-[#000000]">
        <div class="flex justify-center">
        <h1 class="text-white text-8xl font-medium"> Edit Events:</h1>
        </div>


            @csrf
            <button href="{{ url('addevents') }}" type ="submit" class="text-2xl text-white bg-sky-500/50 rounded-lg p-4 m-4">Add events</button>

        <a class="text-2xl" href="{{ route('register') }}" class="ml-4 text-lg text-gray-700 dark:text-white ">Register</a>

    </div>
    <div class="flex justify-center text-white text-4xl mt-8">

    <table class="table-auto">
        <thead>
        <tr class="bg-sky-500/10 ">
            <th class="pr-8">Evenement</th>
            <th class="pr-8">Start</th>
            <th class="pr-8">Einde</th>
            <th class="pr-8">Aantal tickets</th>
            <th class="pr-8">Locatie</th>
            <th class="pr-8">Prijs</th>
            <th class="pr-8">Omschrijving</th>
        </tr>
        </thead>
        <tbody>
        <tr class="bg-sky-500/40">
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
        </tr>
        <tr class="bg-sky-500/20">
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
        </tr>
        <tr class="bg-sky-500/40">
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
        </tr>
        <tr class="bg-sky-500/20">
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
        </tr>
        <tr class="bg-sky-500/40">
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
        </tr>
        <tr class="bg-sky-500/20">
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
        </tr>
        <tr class="bg-sky-500/40">
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
        </tr>
        <tr class="bg-sky-500/20">
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
            <td class="pr-8">data</td>
        </tr>
        </tbody>
    </table>
    </div>
@endsection
