                ...
                @section('content')
                    ...
                    @auth
                        @if(Auth::user()->is_admin)
                            <a href="manage-events"><button class="bg-sky-500/40 rounded-lg p-2 text-3xl text-white font-light m-4 hover:bg-white hover:text-black">Voeg hier een evenement toe</button></a>
                        @endif
                    @endauth
                    ...
                    <div class="flex justify-center">
                        @foreach ($events as $event)
                            <div class="max-w-lg text-2xl bg-[#13293D] rounded-lg p-6 text-white m-8" >
                                ...
                                <img src="{{asset('img/datelogo.png')}}" alt="Evenement logo">
                    ...
                    @auth
                        @if(Auth::user()->is_admin)
                            <form method="POST" action="{{ url('manage-events/' . $event->id) }}" accept-charset="UTF-8" style="display:inline">
                                {{ method_field('DELETE') }}
                                {{ csrf_field() }}
